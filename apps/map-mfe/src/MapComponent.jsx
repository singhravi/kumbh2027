...
// A custom DivIcon that looks like a beautiful modern pin
const customIcon = new L.DivIcon({
    className: 'custom-marker',
    html: `<div style="background-color: #ff5722; width: 24px; height: 24px; border-radius: 50%; border: 3px solid white; box-shadow: 0 4px 6px rgba(0,0,0,0.3);"></div>`,
    iconSize: [24, 24],
    iconAnchor: [12, 12]
});
    ...
    return (
        <div style={{ position: 'absolute', top: 0, left: 0, right: 0, bottom: 0, borderRadius: '12px', overflow: 'hidden', boxShadow: '0 10px 30px rgba(0,0,0,0.1)' }}>
            <MapContainer center={mapCenter} zoom={13} style={{ height: '100%', width: '100%', zIndex: 1 }}>
                ...
                {places.map((place) => (
                    <Marker key={place.id} position={place.coordinates} icon={customIcon}>
                        <Popup>
